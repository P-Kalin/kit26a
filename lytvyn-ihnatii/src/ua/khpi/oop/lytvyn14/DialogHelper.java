...
public class DialogHelper {
	...
	public static void handleAction(ACTION action) throws Exception {
		...
		case clean:
			...
			break;
		case count:
			System.out.println("Введіть таймаут у ms.");
			input = getInput();
			final int timeout = Integer.parseInt(input);
			count(timeout);
			break;
		case read:
		...
		case remove:
			...
			input = getInput();
			final int regNum = Integer.parseInt(input);
	...
	private static void count(int timeout) {
		...
		System.out.format("Час паралельного виконання: %dms\n\n", end - start);
		start = System.currentTimeMillis();
		CountTask.countMinAge(clients);
		CountTask.countMaxAge(clients);
		CountTask.countAvrAge(clients);
		end = System.currentTimeMillis();
		System.out.format("Час послідовного виконання: %dms\n\n", end - start);
	}
}